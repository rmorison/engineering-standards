...
- In this repository's starter kit, the uv entries name the programs they approve, so `uv run op read <reference>`, `uv run env` and `uv run cat <file>` prompt like a bare `op read` or `cat .env`. `Bash(make:*)` still pre-approves every `make` target, including one written on the command line, so `make` reaches the manager CLI as readily as a shell does. `scripts/check-template-kit.mjs` holds both facts as fixtures; this was last confirmed with it on 2026-09-25. The reasoning is in [the kit's README](../templates/README.md#why-allow-names-uv-commands-rather-than-bashuv).
