...
- In this repository's starter kit, the uv entries name the programs they approve, so no allow entry pre-approves `uv run op read <reference>`, `uv run env` or `uv run cat <file>`. A bare `cat .env` in the working tree still runs without a prompt, because `cat` is on Claude Code's built-in list of [read-only commands](https://code.claude.com/docs/en/permissions#read-only-commands). No allow list keeps an agent from reading a file in the tree. Only an `ask` or `deny` rule does, or keeping secret values out of the tree as tier 1 does. `Bash(make:*)` still pre-approves every `make` target, including one written on the command line, so `make` reaches the manager CLI as readily as a shell does. `scripts/check-template-kit.mjs` checks which allow entries match these commands, not whether Claude Code prompts; this was last confirmed with it on 2026-09-25. The reasoning is in [the kit's README](../templates/README.md#why-allow-names-uv-commands-rather-than-bashuv).
