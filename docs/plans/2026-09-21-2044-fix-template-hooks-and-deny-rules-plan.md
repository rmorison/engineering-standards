...
- A2. Resolved by D5. `example-block.py` keeps its `time.sleep(` example rule. Converting it to a Bash command check would make the permission statement self-demonstrating, and would also delete the Write/Edit example the kit teaches. Judged the larger change and the wrong trade for a starter kit.
...
- D5. **`example-block.py` keeps its Write/Edit content rule** rather than becoming a Bash destructive-command blocker. Confirms A2. The cost is accepted: the permission section in `templates/README.md` argues that a `PreToolUse` hook is the real boundary, and the shipped example does not itself demonstrate that. U2 step 2 therefore has to make the argument in prose, and must point at the `PreToolUse` slot as the place such a check goes.
- D6. **The recurrence check runs as a second job in `.github/workflows/docs.yml`**, not as a new workflow file. Confirms A4. The workflow's display name stays "Documentation checks" and is now slightly narrow; renaming a check that branch protection may reference was judged the worse trade.
